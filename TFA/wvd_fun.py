...
from numpy import abs, mean, power, concatenate,arange, shape, array, ceil, zeros, conj, ix_, transpose, append, fft, real, float64, linspace, sqrt
...
def wvd(x, t=None, ifsq=0, alpha = None, trace=0, make_analytic=True,freqlow=0,freqhigh=0.5, Smooth=0, Hemi=0):
    if make_analytic:
        x = hilbert(x)
    else:
        x = array(x)
    ...
    tfrtic = 0.5*arange(N)/float(N)
    tfrsq=[]
    tfrsqtic=[]
    if ifsq:
        Ex = mean(power(abs(x),2))
        Gamma = 1.0e-8*Ex
        dt = t[1] - t[0]
        Dtfr = (-1j/2/pi/dt)*concatenate((tfr[:,1:] - tfr[:,0:-1], array([tfr[:,-1]-tfr[:,-2]]).T),axis = 1) 
